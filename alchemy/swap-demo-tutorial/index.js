...
async function listAvailableTokens() {
  ...
  let tokenList = tokenListJson.tokens;

  // 暂时仅过滤出 WETH 和 DAI 两个代币，方便测试
  tokenList = tokenList.filter((token) => {
    return token.symbol === "WETH" || token.symbol === "DAI";
  });
  ...
  for (const i in tokenList) {
    ...
    let html = `
        <img class="token_list_img" src="${tokenList[i].logoURI} ">
        <span class="token_list_text">${tokenList[i].symbol}</span>`;

    tokenElement.innerHTML = html;
    tokenElement.onclick = () => {
      selectToken(tokenList[i]);
    };
