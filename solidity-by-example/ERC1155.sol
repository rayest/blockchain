...
interface IERC1155TokenReceiver {
    ...
    function onERC1155ReceivedBatch(
        address operator,
        address from,
        uint256[] calldata id,
...
contract ERC1155 is IERC1155 {
    ...
    function safeBatchTransferFrom(
        ...
        if (to.code.length > 0) {
            require(
                IERC1155TokenReceiver(to).onERC1155ReceivedBatch(
                    msg.sender,
                    from,
                    ids,
                    values,
                    data
                ) == IERC1155TokenReceiver.onERC1155ReceivedBatch.selector,
                "unsafe transfer"
            );
        }
    }
...
contract MyMuiltiToken is ERC1155 {
